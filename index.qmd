...
-   **Development**: n=`r n_dev` (`r n_events` events, EPV=`r sprintf("%.1f", epv)`)
    -   Determined using principled criteria (Riley et al. 2020) via `samplesizedev` package
...
1.  **Apparent validation** - Resubstitution on development data (baseline)
2.  **Sample Split** - 70/30 random split
3.  **10-fold Cross-validation** - Stratified folds with pooled predictions
4.  **Bootstrap** - Harrell's optimism correction (200 samples)
...
**Key observation**: Bootstrap and cross-validation show much lower standard deviations than sample splitting, particularly for calibration slope (Bootstrap SD: `r sprintf("%.3f", bootstrap_cal_sd)` vs. Sample Split SD: `r sprintf("%.3f", split_cal_sd)`).
...
**Key observations**: - Bootstrap shows minimal bias across all metrics - Cross-validation slightly underestimates calibration slope (pessimistic) - Sample splitting has low bias on average but very high RMSE due to extreme variability
...
```{r}
#| label: fig-auc
#| fig-cap: "AUC (C-statistic) distributions. Higher is better for discrimination."
#| fig-width: 10
#| fig-height: 6
...
#| fig-cap: "Calibration slope distributions. Perfect calibration = 1.0; overfitting < 1.0. Note the extreme variability of sample splitting."
#| fig-width: 10
#| fig-height: 6
...
These findings align with Smith et al.'s  study comparing internal validation methods. Both studies found: - Bootstrap, cross-validation (with replication), and leave-pair-out methods provide unbiased estimates - Sample splitting and cross-validation without replication show problematic performance - Bootstrap validation is recommended as the primary approach

Our study extends their work by: 1. Higher EPV: We used EPV=`r sprintf("%.1f", epv)` (via principled sample size calculation) vs. their EPV=5 (intentionally low to test challenging scenarios) 2. Multi-metric focus: While Smith et al. emphasized the C-statistic, we evaluated discrimination, calibration, and overall accuracy simultaneously 3. Calibration emphasis: We highlight that sample splitting's variability problem is particularly severe for calibration assessment
...
When to use each method: - Bootstrap: Always preferred when computationally feasible - Cross-validation: Acceptable for very large datasets or complex models where bootstrap is too slow - Sample splitting: Generally not recommended; if used, interpret with extreme caution
